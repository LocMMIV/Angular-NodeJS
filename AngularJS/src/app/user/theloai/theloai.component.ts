...
export class TheloaiComponent implements OnInit {
  ...
  constructor(private searchService: SearchService, private router: Router) {}
    ...
    goToBorrowBook(book: any) {
      // Chuyển hướng đến trang mượn sách với ID của sách
      this.router.navigate(['/muonsach']);
    }
  
    // Phương thức chuyển hướng đến trang chi tiết sách
    goToBookDetail(book: any) {
      // Chuyển hướng đến trang chi tiết sách với ID của sách
      this.router.navigate(['/chitietsach']);
    }
    
  ngOnInit(): void {
    // Load books data ở đây
  }
...
cancel() {
  ...
  if (this.appliedFilters.length > 0) {
    ...
    if (this.selectedSort === 'asc') {
     ...
     // Tìm kiếm sách
     if (this.searchTerm) {
      filtered = this.searchService.searchData(filtered, this.searchTerm);
    }
