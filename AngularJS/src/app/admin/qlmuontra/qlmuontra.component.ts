...
export class QlmuontraComponent implements OnInit {
  ...
  isOverdue(returnDate: string): string {
    ...
    const returnDateObj = new Date(Number(returnDateParts[2]), Number(returnDateParts[1]) - 1, Number(returnDateParts[0])); // Chuyển thành đối tượng Date
    ...
    if (currentDate > returnDateObj) {
      ...
      return `${overdueDays} ngày`; // Trả về số ngày quá hạn
    }
    return 'Không'; // Nếu chưa quá hạn, trả về "Không"
  }
  
  // Hàm này sẽ được gọi trong template để cập nhật "quá hạn"
  getOverdueStatus(row: any): string {
    return this.isOverdue(row.returnDate); // Gọi hàm isOverdue với ngày trả sách
  }
